...
async def show_cart(message: Message, state: FSMContext):
    ...
    await message.answer("Ваша корзина: ", reply_markup=await get_cart_keyboard(db, 0))
...
async def show_product(call: CallbackQuery, state: FSMContext):
    ...
    if img_path:
        await call.message.answer_photo(InputFile(img_path), caption=text,
                                        reply_markup=await get_cart_record_watching_kb(cart_record["number"]))
...
async def show_cart(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Ваша корзина: ", reply_markup=await get_cart_keyboard(db, data["page_num"]))
