...
from keyboards.inline import get_cart_item_watching_kb, get_cart_kb
...
async def show_product(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_caption(text, reply_markup=await get_cart_item_watching_kb(cart_item.amount))
...
async def decrease_amount(call: CallbackQuery, state: FSMContext):
    ...
    if cart_item.amount <= 1:
        ...
        await call.message.edit_reply_markup(reply_markup=await get_cart_item_watching_kb(0))
    else:
        new_number = cart_item.amount - 1
        await cart_item.update(amount=new_number).apply()

    await call.message.edit_reply_markup(reply_markup=await get_cart_item_watching_kb(new_number))
